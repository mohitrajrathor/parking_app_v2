                            ...
                            <div
                                ...
                                <div class="ms-0 ms-sm-3 flex-grow-1 w-100">
                                    ...
                                    <div
                                        ...
                                        <span class="badge" :class="user?.email_confirmed ? 'bg-success' : 'bg-danger'">
                            ...
                            <div class="row g-3">
                                <div class="col-6">
                                    <div class="card bg-success bg-opacity-10 border-0 rounded-3">
                                        <div class="card-body py-3 px-2">
                                            <div class="fs-3 fw-bold text-success mb-1">{{ user?.total_bookings ?? 0 }}</div>
                                ...
                                <div class="col-6">
                                    <div class="card bg-info bg-opacity-10 border-0 rounded-3">
                                        <div class="card-body py-3 px-2">
                                            <div class="fs-3 fw-bold text-info mb-1">{{ user?.active_bookings ?? 0 }}</div>
                                ...
                                <div class="col-6">
                                    <div class="card bg-warning bg-opacity-10 border-0 rounded-3">
                                        <div class="card-body py-3 px-2">
                                            <div class="fs-3 fw-bold text-warning mb-1">₹{{ user?.amount_spended ?? 0 }}</div>
                                ...
                                <div class="col-6">
                                    <div class="card bg-secondary bg-opacity-10 border-0 rounded-3">
                                        <div class="card-body py-3 px-2">
                                            <div class="fs-3 fw-bold text-secondary mb-1">{{ user?.average_rating ? user.average_rating.toFixed(1) : 'N/A' }}</div>
                    ...
                    <div v-for="booking in user.bookings" :key="booking.id" class="mb-3">
                        <div class="card border-0 rounded-4 position-relative overflow-hidden">
                            <div class="card-body p-4">
                                ...
                                <div class="mb-2">
                                    <span class="p-2 rounded-3 bg-dark-subtle bg-opacity-25 text-dark fw-semibold me-2">Slot: {{ booking.slot?.serial_id }}</span>
                                </div>
                                <div class="mb-3">
                                        ...
                                        <table class="table table-bordered table-sm align-middle mb-0">
                                            ...
                                            <tbody>
                                                <tr v-for="charge in booking.charges" :key="charge.id">
                                                    ...
                                                    <td class="text-muted small">{{ formatDate(charge.paid_at) }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                ...
                                <div class="mt-3 text-end">
                                    <button @click="leaveParkingRedirect(booking?.id, booking?.parking?.id)" class="btn btn-danger btn-sm rounded-pill px-4">Leave</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card border-0 rounded-4 bg-light-subtle">
                            <div class="card-body p-4">
                                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 gap-2">
                                    ...
                                    <div class="text-muted small">Start: {{ formatDate(history.start_time) }}<span v-if="history.leave_time"> | End: {{ formatDate(history.leave_time) }}</span></div>
                                </div>
                                <div class="mb-2">
                                    <span class="p-2 rounded-3 bg-dark-subtle bg-opacity-25 text-dark fw-semibold me-2">Slot: {{ history.slot?.serial_id }}</span>
                                ...
                                <div class="mb-2">
                                    ...
                                    <span class=" text-dark fw-bold ms-3">{{ getParkingHours(history.start_time, history.leave_time) }} Hours</span>
                                </div>
                        ...
                        <div class="col-6 col-md-3">
                            <div class="text-center">
                                ...
                                <div class="progress mt-2" style="height: 6px;">
                                    <div class="progress-bar bg-success" role="progressbar" :style="{width: (user?.total_bookings ? Math.min(user.total_bookings * 20, 100) : 0) + '%'}"></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            <div class="text-center">
                                ...
                                <div class="small text-muted">Active Bookings</div>
                                <div class="progress mt-2" style="height: 6px;">
                                    <div class="progress-bar bg-info" role="progressbar" :style="{width: (user?.active_bookings ? Math.min(user.active_bookings * 20, 100) : 0) + '%'}"></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            <div class="text-center">
                                ...
                                <div class="small text-muted">Total Spent</div>
                                <div class="progress mt-2" style="height: 6px;">
                                    <div class="progress-bar bg-warning" role="progressbar" :style="{width: (user?.amount_spended ? Math.min(user.amount_spended, 100) : 0) + '%'}"></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            <div class="text-center">
                                <div class="fs-2 fw-bold text-secondary">{{ user?.average_rating ? user.average_rating.toFixed(1) : 'N/A' }}</div>
                                <div class="small text-muted">Average Rating</div>
                                <div class="progress mt-2" style="height: 6px;">
                                    <div class="progress-bar bg-secondary" role="progressbar" :style="{width: (user?.average_rating ? user.average_rating * 20 : 0) + '%'}"></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
import UserLogo from "./user/UserLogo.vue";
