...
def create_parking(args):
    ...
    try:
        ...
        if args.get("slots_num") > 200 or args.get("slots_num") < 10:
            raise APIError(
        ...
        if (
            Parking.query.filter_by(name=args["name"]).first()
...
def update_parking(args):
    ...
    """
    try:
        parking = Parking.query.get(int(args.get("id")))
        ...
        if args.get("slots_num") > parking.slots_num:

            slots = parking.slots
            # slots nums to add
            add_slots_num = int(args.get("slots_num")) - len(slots)

            i = 1
            while add_slots_num > 0:
                if not Slot.query.filter_by(serial_id=f"PR{parking.id}SL{i}").first():
                    db.session.add(
                        Slot(parking_id=parking.id, serial_id=f"PR{parking.id}SL{i}")
                    )
                    add_slots_num -= 1
            ...
            parking.slots_num = args.get("slots_num")
            db.session.commit()

        if args.get("slots_num") < parking.slots_num:

            if args.get("slots_num") < 10:
                return {
            ...
            for slot in slots:
                ...
                if len(to_dlt) == int(parking.slots_num - args.get("slots_num")):
                    for sl in to_dlt:
...
def delete_parking(args):
    ...
    try:
        ...
        if not Slot.query.filter_by(is_occupied=True).all():
